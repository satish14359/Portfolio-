...
inquiryForm.addEventListener('submit',async event=>{event.preventDefault();const status=document.querySelector('.form-status');const button=inquiryForm.querySelector('button');button.disabled=true;button.textContent='Sending…';status.textContent='';try{const response=await fetch('https://formspree.io/f/mrenzgnb',{method:'POST',body:new FormData(inquiryForm),headers:{Accept:'application/json'}});if(!response.ok)throw new Error('Submission failed');inquiryForm.reset();status.textContent='Thank you — your message has been sent successfully.'}catch(error){status.textContent='Your message could not be sent. Please try again or contact Satish on LinkedIn.'}finally{button.disabled=false;button.innerHTML='Send inquiry <span>↗</span>'}});
